...
function reserveNodes {
    ...
    while [ $reservationFail == "true" ]; do
	    ...
	    if [ -z "$RES_ID" ]; then
		    ...
		    next=${next//\"/} # Replace all the occurences of '"' with emtpy string
		    next=${next//\'/} # Replace all the occurences of "'" with empty string
...
sed -i "s/nodes=.*/${nodeStr}/g" rubis-configuration.sh
sed -i "s/servers=.*/${servers}/g" rubis-configuration.sh
sed -i "s/clients=.*/${clients}/g" rubis-configuration.sh
